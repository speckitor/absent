...
void client_create(state_t *s, xcb_window_t wid) {
  xcb_map_window(s->c, wid);
	...
	xcb_change_window_attributes(s->c, wid, XCB_CW_BORDER_PIXEL | XCB_CW_EVENT_MASK, value_list);

	xcb_get_geometry_reply_t *attrs = xcb_get_geometry_reply(s->c, xcb_get_geometry(s->c, wid), NULL);

  xcb_grab_button(s->c, 0, wid,
									XCB_EVENT_MASK_BUTTON_PRESS | XCB_EVENT_MASK_BUTTON_RELEASE | XCB_EVENT_MASK_BUTTON_MOTION, 
									XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC, 
                  s->root, XCB_NONE, 1, BUTTON_MOD);
  xcb_grab_button(s->c, 0, wid, 
									XCB_EVENT_MASK_BUTTON_PRESS | XCB_EVENT_MASK_BUTTON_RELEASE | XCB_EVENT_MASK_BUTTON_MOTION, 
									XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC, 
                  s->root, XCB_NONE, 3, BUTTON_MOD);
...
void client_resize(state_t *s, client_t *cl, xcb_motion_notify_event_t *e) {
	...
	switch (s->m->resizingcorner) {
		...
		case TOP_RIGHT:
			...
			value_mask = XCB_CONFIG_WINDOW_Y | XCB_CONFIG_WINDOW_WIDTH | XCB_CONFIG_WINDOW_HEIGHT;
			value_list[0] = new_y;
		...
		case BOTTOM_LEFT:
			...
			value_mask = XCB_CONFIG_WINDOW_X | XCB_CONFIG_WINDOW_WIDTH | XCB_CONFIG_WINDOW_HEIGHT;
			value_list[0] = new_x;
			value_list[1] = new_width;
			value_list[2] = new_height;
			xcb_configure_window(s->c, cl->wid, value_mask, value_list);
...
int client_contains_cursor(state_t *s, client_t *cl) {
	...
	if (reply) {
		if (reply->root_x >= cl->x && 
			reply->root_y >= cl->y && 
			reply->root_x <= cl->x + cl->width && 
			reply->root_y <= cl->y + cl->height) ret = 1; 	
	}
